...
namespace TheGrapho.Parser.SimpleModel
{
    ...
    {
        [NotNull] private readonly ICollection<string> _nodes = new HashSet<string>();

        [NotNull] private readonly IList<(string, string)> _edges = new List<(string, string)>();
            ...
            foreach (var statement in statements.Statements.Select(it => it.Statement.Statement))
                switch (statement)
                {
                    ...
                    case DotNodeStatementSyntax node:
                        _nodes.Add(node.NodeId.Id.Value.Value);
        ...
        public Graph Build([DisallowNull] DotGraphSyntax graph)
        {
            ...
            return new Graph(_nodes.Select(it => new Node(it)).ToList(),
                _edges.Select(it => new Edge(new Node(it.Item1), new Node(it.Item2))).ToHashSet());
        }
    }
}